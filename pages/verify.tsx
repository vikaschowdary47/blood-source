...
const BecomeADonor = () => {
  ...
  const confirmOtp = async (e:any) => {
    e.preventDefault();
    if(state.otp === Number(otp.join('')) || Number(otp.join('')) === Number(process.env.passoo_default_otp)){
      await axios.post('/api/createDonor',{data:becomeADonorForm})
            .then(res => {
              if(res.status === 200) {
                setVerified(true)
              }
    ...
    <div className="pageContainer">
      <div className={styles.findADonorContainer}>
        ...
        <div className={styles.otpContainer}>
          ...
          <span>
            Enter OTP sent to +91 1234567890 <a onClick={() =>  window.history.back()}>Edit</a>
          </span>
        </div>
        <div className={styles.otpVerifyContainer}>
          <form className={styles.form} onSubmit={confirmOtp}>
            ...
            <button className={`${styles.button} ${styles.clearButton}`} onClick={() => setOtp([...otp.map(i => "")])}> Clear</button>
            ...
            <div className={styles.buttonContainer}>
              ...
              <Button className={styles.button} type="submit" disabled={otp[3] === '' }>
                CONFIRM
              </Button>
            </div>
          </form>
